...
let get_graph_bounding_box stmt_list =
  let rec iter = function
    ...
    | Dot.Stmt_attr (Dot.Attr_graph attr_list) :: q -> (
        ...
        | Some (Dot.Simple_id v) | Some (Dot.Double_quoted_id v) -> (
  ...
  try
    ...
    let rec iter acc = function
      ...
      | stmt :: q -> (
          ...
          | Dot.Stmt_subgraph g -> iter acc (g.Dot.sub_stmt_list @ q)
          | Dot.Stmt_equals _ | Dot.Stmt_edge _ | Dot.Stmt_attr _ -> iter acc q
          )
    in
